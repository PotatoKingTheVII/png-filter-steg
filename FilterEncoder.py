...
for i in range(0,len(TextIn)):
    ...
    TextList += Temp
TextList = TextList.replace("1", "1")
TextList = TextList.replace("0", "0")
...
MaxLength = math.floor(height/8)
print("*Max message length is " +str(MaxLength) + " characters*")
if(len(TextIn)>MaxLength):
    print("Message too long, excess will be ignored")
print("\nPreprocessing image...")
for i in range(img.size[0]*img.size[1],0,-1):
    x=i%img.size[0]
    y=(math.floor(i/img.size[0]))

    try:
        filterbyte = TextList[y]
    except:
        filterbyte = 0

    if(filterbyte=="1"):
        if(x>0):
            pixels[x,y] = tuple(numpy.mod((numpy.subtract(pixels[x,y], pixels[x-1,y])),256))
